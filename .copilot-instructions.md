...
- **Component Naming**: Use PascalCase for component names (e.g., `AssCanvas.razor`)
...
- Store asset configurations in static dictionaries (`ClicksRequired`, `AssPoints`, etc.)
- Organize assets by logical groupings in `Client/wwwroot/images/[AssType]/`
- Each ass type uses a single `entire_ass.png` that is eaten via canvas chunk erasure
- Hole state uses `hole.png` (all types except Boney); Boney has no hole
...
- Use `ILocalStorageService` for client-side persistence (daily challenges, progress, settings)
...
- **Services** (registered in `Program.cs`):
  - `StatsService` — global stats tracking (clicks, asses eaten, rounds, time)
  - `ProgressService` — per-ass-type progress and perks (10/25/50/100 eaten thresholds)
  - `CollectionService` — Assdex unlock tracking
  - `DailyChallengeService` — daily challenge persistence and completion
  - `VersionCheckService` — app version checking and update prompts
  - `LeaderboardService` / `LocalLeaderboardService` — score submission and retrieval
  - `SettingsService` — user preferences (sound, etc.)
...
- **Modal backgrounds**: Never add `background` or gradient rules to modal `.razor.css` files. The global `.modal-content` rule in `app.css` (`background: #1a1a2e`) applies to all modals. Only inner content cards may use `rgba(255,255,255,0.05)` frosted-glass styling.

### Component Patterns

- **Modals**: Extend `BSModal` base component for consistent modal behavior; use `modal-body` only (no `modal-header`/`modal-footer`)
- **Navigation**: Use centralized navigation component (`Navbar.razor`)
- **Game Elements**: Use `AssCanvas.razor` for the interactive eating display (canvas-based chunk erasure)
